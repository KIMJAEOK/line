$(function () {
    ...
    var telText_MO = "듣고싶은 목소리 <br /> 망설이지 마세요."
    ...
    var gapH = 0; // 가로 폭에 따른 높이값 변수지정 (초기화) / pc -> 70px / mobile -> 50px
    ...
    // or var arrContH = []; 배열을 선언하면서 값을 넣을 때 씀
    ...
    // .offset() - 문서를 기준으로 선택한 요소의 가로/세로의 떨어진 위치의 좌표값 변환 및 변경시 사용
    ...
    $btnAuto.data("state", "on"); // 버튼에 내부 데이터 설정 - 재생중인 상태와 정지상태
    ...
    /* setIndicator 로 함수 저장 - 인디케이터 자체인 a라는 태그로 변수 지정된 
    indicator의 부모인 li에 on클래스 삽입하고, 나머지는 on 삭제  */
   ...
   만약, oldIdx가 nowIdx와 다르면, 슬라이드를 실행시키고, 자동재생 상태변환 함수호출, 링크차단 함.    */
    ...
    // null: 변수에 지정된 데이터를 지우고자 할 때 사용 
    // Math.max: 최대값 반환
    // Math.min: 최소값 반환
    // Math.round(num): 반올림값 반환
    // Math.floor(num): 소수점에서 무조건 내림
    // Math.ceil(num): 소수점에서 무조건 올림
